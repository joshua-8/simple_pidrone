...
class FlightController(object):
    ...
    def new_fly_command_callback_twist(self, msg):
        if self.curr_mode == 'FLYING':
            self.command = [
                1500+500*msg.linear.x, 1500+500 * msg.linear.y, 1500+500*msg.angular.x, 1500+500*msg.linear.z]
    ...
    def update_imu_message(self):
        ...
        quaternion_array = [previous_quaternion.x, previous_quaternion.y,
                            previous_quaternion.z, previous_quaternion.w]
        previous_roll, previous_pitch, previous_heading = tf.transformations.euler_from_quaternion(
            quaternion_array)
        ...
        g = 9.8
        lin_acc_x_drone_body = lin_acc_x_drone_body + \
            g*np.sin(roll)*np.cos(pitch)
        lin_acc_y_drone_body = lin_acc_y_drone_body + \
            g*np.cos(roll)*(-np.sin(pitch))
        lin_acc_z_drone_body = lin_acc_z_drone_body + \
            g*(1 - np.cos(roll)*np.cos(pitch))
...
def main():
    ...
    imupub = rospy.Publisher(
        '/pidrone/imu', Imu, queue_size=1, tcp_nodelay=False)
    batpub = rospy.Publisher('/pidrone/battery', Battery,
                             queue_size=1, tcp_nodelay=False)
    fc.modepub = rospy.Publisher(
        '/pidrone/mode', Mode, queue_size=1, tcp_nodelay=False)
    ...
    rospy.Subscriber("/pidrone/desired/twist", Twist,
                     fc.new_fly_command_callback_twist)

    # heartbeat subscribers
    rospy.Subscriber("/pidrone/range", Range, fc.heartbeat_infrared_callback)
    rospy.Subscriber("/pidrone/heartbeat/web_interface",
                     Empty, fc.heartbeat_web_interface_callback)
    ...
    # /pidrone/position_control      std_msgs/Bool

  #     name : '/pidrone/desired/twist',    #     messageType : 'geometry_msgs/Twist'

    #     name : '/pidrone/desired/pose',    #     messageType : 'geometry_msgs/Pose'
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if fc.shouldIDisarm():
                print("SHOULD DISARM!!! mode", fc.curr_mode)
                break
